...
BEGIN
  ...
  IF v_stock_item_id IS NULL AND NEW.payment_type IN ('APOSTILA', 'MATRICULA') THEN
    -- Fallback 2: match by category for APOSTILA/MATRICULA types
    SELECT id INTO v_stock_item_id
    FROM public.stock_items
    WHERE unit_id = NEW.unit_id
      AND active = true
      AND LOWER(category) = LOWER(NEW.payment_type)
    LIMIT 1;
  END IF;
