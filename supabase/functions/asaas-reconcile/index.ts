...
function mapBillingType(method?: string | null) {
  ...
  if (raw === "CARD" || raw === "CREDIT_CARD" || raw === "CARTAO")
    return "CREDIT_CARD";
  return "BOLETO";
}
...
async function upsertInconsistency(
  admin: AdminClient,
  ...
  try {
    ...
    else if (issue.asaas_payment_id)
      q = q.eq("asaas_payment_id", issue.asaas_payment_id);
    else return;
  ...
  for (const filter of filters) {
    ...
    while (true) {
      ...
      if (!res.ok) {
        stats.errors++;
        console.error(
          "[asaas-reconcile] erro ao listar cobranças",
          res.status,
          json,
        );
        break;
...
async function fetchAsaasByExternalReference(
  baseUrl: string,
  apiKey: string,
  externalReference: string,
) {
...
async function searchCustomersByDoc(
  baseUrl: string,
  apiKey: string,
  doc: string,
) {
...
async function ensureCustomer(
  ...
  if (customerId) {
    const check = await fetch(`${baseUrl}/customers/${customerId}`, {
      headers: { access_token: apiKey },
    }).catch(() => null);
  ...
  if (!validDoc(doc))
    throw new Error(
      `CPF/CNPJ inválido para ${responsible.full_name || responsible.id}`,
    );
  ...
  if (found.length > 0) {
    ...
    if (found.length > 1) {
      ...
      report.push({
        type: "ASAAS_DUPLICATE_CUSTOMER",
        unit: unitName,
        responsible: responsible.full_name,
        message: `${found.length} clientes com o mesmo CPF/CNPJ no Asaas; usando ${customerId}.`,
      });
    }
    await admin
      .from("profiles")
...
async function attachAsaasToLocal(
  admin: AdminClient,
  payment: any,
  a: AsaasPayment,
  action: string,
  ...
  if (a.billingType)
    update.payment_method =
      a.billingType === "CREDIT_CARD" ? "CARD" : a.billingType;
  if (isPaid && payment.status !== "PAID") {
    update.status = "PAID";
    update.paid_at = paidAtFromAsaas(a);
    update.final_value = paidValueFromAsaas(
      a,
      payment.final_value ?? payment.value,
    );
    stats.paid_synced++;
  } else if (
    CANCELLED_ASAAS.has(String(a.status)) &&
    !LOCAL_PAID.has(payment.status)
  ) {
  ...
  report.push({
    ...
    message: `Parcela local vinculada à cobrança Asaas ${a.id}.`,
  });
  await logPaymentSync(admin, {
    payment_id: payment.id,
    unit_id: payment.unit_id,
...
async function syncPaidStatus(
  ...
  stats.paid_synced++;
  report.push({
    type: "AUTO_RECONCILE_PAID",
    unit: unitName,
    payment_id: payment.id,
    asaas_payment_id: a.id,
    responsible: responsibleName,
    message: "Baixa automática aplicada a partir do status pago no Asaas.",
  });
  await logPaymentSync(admin, {
    payment_id: payment.id,
    unit_id: payment.unit_id,
...
async function cancelDuplicate(
  ...
  if (LOCAL_PAID.has(String(payment.status))) {
    ...
    await upsertInconsistency(admin, {
      ...
      details: {
        reason:
          "duplicidade paga não cancelada automaticamente para preservar cobrança paga válida",
      },
    });
    return;
  }
  ...
  if (payment.asaas_payment_id && a && !PAID_ASAAS.has(String(a.status))) {
    const res = await fetch(
      `${unit.asaas_base_url || "https://api.asaas.com/v3"}/payments/${payment.asaas_payment_id}`,
      {
        method: "DELETE",
    ...
    if (!res.ok && !CANCELLED_ASAAS.has(String(a.status))) {
      ...
      await logPaymentSync(admin, {
        ...
        action: "AUTO_CANCEL_DUPLICATE_ASAAS",
        success: false,
        response_payload: json,
        error_message:
          json?.errors?.[0]?.description ||
          json?.message ||
          "Falha ao cancelar duplicada no Asaas",
      });
      return;
    }
    asaasCancelled = true;
  ...
  const { error } = await admin
    ...
    .update({
      ...
      sync_error: null,
      corrected_automatically: true,
      emission_status: payment.asaas_payment_id
        ? "CANCELLED_DUPLICATE"
        : payment.emission_status,
    })
    .eq("id", payment.id)
    .neq("status", "PAID");
  ...
  if (error) {
    ...
    await logPaymentSync(admin, {
      ...
      responsible_id: payment.responsible_id,
      asaas_payment_id: payment.asaas_payment_id,
      action: "AUTO_CANCEL_DUPLICATE_LOCAL",
      success: false,
      error_message: error.message,
    });
    return;
  ...
  report.push({
    type: "AUTO_CANCEL_DUPLICATE",
    unit: unit.name,
    payment_id: payment.id,
    asaas_payment_id: payment.asaas_payment_id,
    responsible: responsibleName,
    message: `Duplicidade cancelada no sistema${asaasCancelled ? " e no Asaas" : ""}; histórico preservado.`,
  });
  await logPaymentSync(admin, {
    payment_id: payment.id,
    unit_id: payment.unit_id,
    ...
    success: true,
    response_payload: { asaas_cancelled: asaasCancelled },
  });
  await upsertInconsistency(admin, {
    payment_id: payment.id,
    unit_id: payment.unit_id,
    ...
    system_due_date: payment.due_date,
    details: {
      resolution: "cancelada automaticamente por reconciliação",
...
async function createMissingAsaasCharge(
  admin: AdminClient,
  payment: any,
  unit: any,
  responsible: any,
  ...
  const provider = String(
    payment.payment_provider || payment.gateway || "ASAAS",
  ).toUpperCase();
  if (
    provider !== "ASAAS" ||
    String(payment.payment_method || "").toUpperCase() === "DINHEIRO"
  )
    return;

  const value = Number(payment.final_value ?? payment.value ?? 0);
  const originalValue = Number(
    payment.original_value ?? payment.value ?? value,
  );
  if (!Number.isFinite(value) || value < 10) return;

  const baseUrl = unit.asaas_base_url || "https://api.asaas.com/v3";

  const already = await fetchAsaasByExternalReference(
    baseUrl,
    unit.asaas_api_key,
    payment.id,
  );
  const activeExisting = already.find(
    (a) => !CANCELLED_ASAAS.has(String(a.status)),
  );
  if (activeExisting) {
    await attachAsaasToLocal(
      admin,
      payment,
      activeExisting,
      "AUTO_ATTACH_EXTERNAL_REFERENCE",
      stats,
      report,
      unit.name,
  ...
  try {
    ...
    await admin
      ...
      .update({
        ...
        emission_error_code: "CUSTOMER_ERROR",
    ...
    await logPaymentSync(admin, {
      ...
      unit_id: payment.unit_id,
      responsible_id: payment.responsible_id,
      action: "AUTO_CREATE_MISSING_CHARGE",
      success: false,
  ...
  const update = {
    ...
    payment_method:
      json.billingType === "CREDIT_CARD"
        ? "CARD"
        : json.billingType || payment.payment_method || "BOLETO",
    status: PAID_ASAAS.has(String(json.status)) ? "PAID" : "PENDING",
    paid_at: PAID_ASAAS.has(String(json.status))
      ? paidAtFromAsaas(json)
      : payment.paid_at,
    final_value: PAID_ASAAS.has(String(json.status))
      ? paidValueFromAsaas(json, value)
      : payment.final_value,
    gateway: "ASAAS",
  ...
  report.push({
    type: "AUTO_CREATE_MISSING_CHARGE",
    unit: unit.name,
    payment_id: payment.id,
    asaas_payment_id: json.id,
    responsible: responsible?.full_name,
    message: "Cobrança ausente criada no Asaas e vinculada à parcela local.",
  });
  await logPaymentSync(admin, {
    payment_id: payment.id,
    unit_id: payment.unit_id,
...
Deno.serve(async (req) => {
  ...
  const isScheduled =
    body?.scheduled === true ||
    req.headers.get("x-internal-key") === serviceRoleKey;
  const daysBack = Math.max(1, Math.min(365, Number(body?.days_back) || 120));
  const filterUnitId =
    typeof body?.unit_id === "string" ? body.unit_id : undefined;
  const repairDuplicates = body?.repair_duplicates !== false;
  const emitMissing = body?.emit_missing !== false;
  const maxCreate = Math.max(
    0,
    Math.min(1000, Number(body?.max_create) || 300),
  );
  ...
  for (const unit of units ?? []) {
    ...
    try {
        ...
        fetchAll<any>(
          (from, to) =>
            admin
              .from("profiles")
              .select(
                "id, full_name, cpf, phone, email, asaas_customer_id, unit_id, active",
              )
              .eq("unit_id", unit.id),
          1000,
        ),
      ...
      const responsibleByCustomer = new Map(
        responsibles
          .filter((r) => r.asaas_customer_id)
          .map((r) => [r.asaas_customer_id, r]),
      );
      const localById = new Map(localPayments.map((p) => [p.id, p]));
      const localByAsaas = new Map(
        localPayments
          .filter((p) => p.asaas_payment_id)
          .map((p) => [p.asaas_payment_id, p]),
      );
      ...
      for (const a of asaasPayments) {
        ...
        if (local) {
          await syncPaidStatus(
            ...
            report,
            unit.name,
            responsibleById.get(local.responsible_id)?.full_name,
          );

          const valueMismatch =
            cents(local.original_value ?? local.value) !== cents(a.value);
          const dueMismatch = Boolean(
            a.dueDate && local.due_date && a.dueDate !== local.due_date,
          );
          if (valueMismatch || dueMismatch) {
            await upsertInconsistency(admin, {
              ...
              responsible_name:
                responsibleById.get(local.responsible_id)?.full_name ?? null,
              ...
              details: {
                reason:
                  "divergência detectada durante reconciliação automática",
                asaas: { value: a.value, dueDate: a.dueDate },
              },
            });
          }
          continue;
        ...
        if (externalReference && localById.has(externalReference)) {
          ...
          if (candidate && !candidate.asaas_payment_id) {
            attached = await attachAsaasToLocal(
              admin,
              candidate,
              a,
              "AUTO_ATTACH_BY_EXTERNAL_REFERENCE",
              stats,
              report,
              unit.name,
              responsibleById.get(candidate.responsible_id)?.full_name,
            );
          }
        }
        ...
        if (!attached && a.customer) {
          ...
          if (responsible) {
            const candidate = localPayments.find(
              (p) =>
                !p.asaas_payment_id &&
            ...
            if (candidate) {
              attached = await attachAsaasToLocal(
                ...
                candidate,
                a,
                "AUTO_ATTACH_BY_CUSTOMER_DATE_VALUE",
                stats,
                report,
                unit.name,
                responsible.full_name,
        ...
        if (!attached) {
          ...
          await upsertInconsistency(admin, {
            ...
            details: {
              reason:
                "cobrança existe no Asaas mas não foi localizada no sistema",
      ...
      if (repairDuplicates) {
        ...
        for (const arr of groups.values()) {
          ...
          arr.sort((a, b) => {
            const score = (p: any) =>
              LOCAL_PAID.has(p.status) ? 0 : p.asaas_payment_id ? 1 : 2;
            return (
              score(a) - score(b) ||
          ...
          const keep = arr[0];
          const paidCount = arr.filter((p) =>
            LOCAL_PAID.has(String(p.status)),
          ).length;
          for (const duplicate of arr.slice(1)) {
            if (paidCount > 1 && LOCAL_PAID.has(String(duplicate.status))) {
              await cancelDuplicate(
                admin,
                duplicate,
                asaasById.get(duplicate.asaas_payment_id),
                unit,
                stats,
                report,
                responsibleById.get(duplicate.responsible_id)?.full_name,
              );
              continue;
            }
      ...
      if (emitMissing) {
        ...
        const freshNoAsaas = localPayments
          .filter(
            (p) =>
              ...
              p.status === "PENDING" &&
              String(
                p.payment_provider || p.gateway || "ASAAS",
              ).toUpperCase() === "ASAAS",
          )
        ...
        for (const p of freshNoAsaas) {
          ...
          await createMissingAsaasCharge(
            admin,
            p,
            unit,
            responsible,
      ...
      if (/serra verde/i.test(unit.name)) {
        const fabianos = responsibles.filter((r) =>
          /fabiano/i.test(r.full_name || ""),
        );
        specialFocus.serra_verde = {
          unit_id: unit.id,
          local_payments_scanned: localPayments.length,
          asaas_charges_seen: asaasPayments.length,
          fabiano_profiles: fabianos.map((f) => ({
            id: f.id,
            name: f.full_name,
            cpf: f.cpf,
            asaas_customer_id: f.asaas_customer_id,
          })),
          pending_without_asaas_after_scan: localPayments.filter(
            (p) => !p.asaas_payment_id && p.status === "PENDING",
          ).length,
        };
      }
    } catch (e) {
  ...
  return respond({
    ...
    errors_remaining:
      stats.errors +
      stats.skipped_paid_duplicates +
      stats.webhook_failures_marked_for_review,
    report,
    special_focus: specialFocus,
  });
});
